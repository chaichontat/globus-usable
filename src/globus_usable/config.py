...
DEFAULT_LOCAL_ENDPOINT_ID_PLACEHOLDER = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
...
def render_init_config_toml(
    *,
    remotes: dict[str, str],
    default_remote: str | None = None,
    local_endpoint_id: str | None = None,
) -> str:
    ordered = sorted(remotes.items(), key=lambda kv: kv[0])
    default_remote_name = default_remote or (ordered[0][0] if ordered else "dsai")

    lines: list[str] = []
    lines.append("# Generated by globus-usable; edit as needed.")
    lines.append("")
    lines.append("[local]")
    lines.append(f'endpoint_id = "{local_endpoint_id or DEFAULT_LOCAL_ENDPOINT_ID_PLACEHOLDER}"')
    lines.append("")

    for name, endpoint_id in ordered:
        lines.append(f"[remotes.{name}]")
        lines.append(f'endpoint_id = "{endpoint_id}"')
        lines.append("")

    lines.append("[defaults]")
    lines.append(f'default_remote = "{default_remote_name}"')
    lines.append('sync_level = "mtime"')
    lines.append("poll_interval_min = 2")
    lines.append("poll_interval_max = 30")
    lines.append("")
    return "\n".join(lines)
